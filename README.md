...
## Built-in tunnel
This repository now also includes the USTPS tunnel implementation directly, so you do not need the separate `USTPS-Tunnel` repository just to test a point-to-point private IPv4 tunnel.
...
Server key:
```bash
python3 tunnel_keygen.py --name server --mode server --out-dir keys
```

Client key:
```bash
python3 tunnel_keygen.py --name client --mode client --out-dir keys
```
...
sudo python3 tunnel_server.py \
  --bind-ip 0.0.0.0 \
  --bind-port 5400 \
  --server-private-key keys/server.key \
  --client-public-key "<CLIENT_PUBLIC_KEY_BASE64>" \
  --password "MY_TUNNEL_PASSWORD" \
  --cipher aes-256-gcm \
  --tun-name ustpst0 \
  --tun-local-ip 10.77.0.1 \
  --tun-peer-ip 10.77.0.2 \
  --mtu 1100
```
...
sudo python3 tunnel_client.py \
  --server-ip <SERVER_IP> \
  --server-port 5400 \
  --client-private-key keys/client.key \
  --password "MY_TUNNEL_PASSWORD" \
  --cipher aes-256-gcm \
  --tun-name ustpt0
```
...
Notes:
- This tunnel is still based on the same USTPS transport philosophy.
- It is best treated as experimental.
- For heavy HTTP/HLS tunneling, expect more sensitivity than the direct video mode in `server.py` / `client.py`.
